...
function DangDangRESTAPI(baseURL, method, treeMap){
	this.baseURL = baseURL;
	this.method = method;
	this.ht = treeMap;
}

DangDangRESTAPI.prototype.put = function (key, value) {
		this.ht.put(key, value);
}

DangDangRESTAPI.prototype.encrypt = function (app_secret) {
		var orgin = new StringBuffer();

		var map = new TreeMap();
		map.putAll(this.ht);
        
        //append value for each key
		var iter = map.keySet().iterator();
		while (iter.hasNext()) {
			var name = String.valueOf(iter.next());
			if (new String(map.get(name)).length() > 0) {
				orgin.append(map.get(name));
			}
		}
...
DangDangRESTAPI.prototype.URLEncode = function (rp) {
	...
	var url = this.baseURL + "/" + this.method + ".php?" + request_params.toString().substring(1);
